    ...
    <section className="shell py-20 sm:py-24 lg:py-32">
      ...
      <ol className="mt-16 border-t border-ink lg:mt-24">
        {items.map((item, index) => (
          ...
          >
            <div className="grid grid-cols-12 items-baseline gap-x-6 gap-y-2 py-6 sm:py-8">
              ...
              <h3 className="col-span-10 font-display text-[clamp(1.375rem,4.2vw,2.75rem)] font-extrabold uppercase leading-[0.95] tracking-[-0.04em] transition-colors duration-200 group-hover:text-accent sm:col-span-6">
                {item.title}
              </h3>

              <p className="col-span-12 max-w-md text-[0.8125rem] leading-relaxed text-ink-60 sm:col-start-8 sm:col-span-5">
                {item.description}
              </p>

              <span
                aria-hidden
                className="col-span-12 hidden text-right font-mono text-ink-20 transition-all duration-300 group-hover:translate-x-1 group-hover:text-accent sm:col-span-12 lg:block"
              >
                →
              </span>
            </div>
